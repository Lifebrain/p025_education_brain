...
label_lh_stats = paste(
  Sys.getenv("OUTPUT_DIR"),
  "02_mass-univariate_analyses/07_model03_overlap_lb_ukb/stats/combined_binarized.sig.lh.th20.abs.sig.masked.volume_C.mgh.label.stats",
  sep="/"
)

lh_volume <- read.table(label_lh_stats,header=FALSE)

dat$value = lh_volume$V1
...
ggplot(dat, aes(x = age, y = value, group = id,color=education_group)) + 
  ...
  ggtitle("UKB: Mean cortical volume of regions where the offset effect coincides between UKB and LB")
...
# Save plot
ggsave(filename = "04_UKB_cortical_volume_all.png", width = 12, height = 8, units = "cm")
...
# other variable like sex, scanner, etc, since the purpose here
# is visualization only.
mod2 <- gamm(value ~ education_group + s(age) + s(age,by=education_group) + sex, data = dat, random = list(id =~ 1))
summary(mod2$gam) 
# Plot the estimated function. Very nonlinear.
# This uses R's built-in plotting tool.
...
ggplot(grid, aes(x = age, y = estimate, group=education_group,color=education_group, ymin=lower_ci, ymax=upper_ci)) +
  ...
  # Save plot
ggsave(filename = "04_UKB_cortical_volume_covariates_included.png", width = 12, height = 8, units = "cm")
ggsave(filename = "04_UKB_cortical_volume_covariates_included.svg", width = 12, height = 8, units = "cm")
