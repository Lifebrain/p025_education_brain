...
awk '{if(NR>1)print "INPUT",$1,"Class1",$4}' $QDEC_DATA_DIR/sorted.qdec.table.dat >> $dummy_fsgd_file

echo "0 1" > $dummy_mtx_file

fwhm_sm15_lh=$CONCAT_DATA_DIR/lh.${measure}.fsaverage.sm15.mgh
fwhm_sm15_rh=$CONCAT_DATA_DIR/rh.${measure}.fsaverage.sm15.mgh
...
for hemi in "lh" "rh"
do
    for sig_path in $(ls $data_to_correct/p_maps/*$hemi*${measure}*C.mgh)
    do
...
for glmdir in $(ls -d $data_to_correct/p_maps/glmdir*)
do
    for p_val in "1.3" "2.0" "3.0"
    do
        mri_glmfit-sim \
        --glmdir ${glmdir} \
        --cache ${p_val} abs \
        ...
        ln -sf ${glmdir}/slope/cache.th${p_val/./}.abs.sig.masked.mgh \
             $data_to_correct/p_maps/sig.${hemi}.th${p_val/./}.abs.sig.masked.${measure}_C.mgh
    done
done
