...
[m,n] = size(Qdec);
scanner = Qdec(2:end,n-1);

Qdec = rmQdecCol(Qdec, n-1);                    % remove the scanner column from the table
...
% Change order in fieldnames, so scanner is last:
fieldNames(n+1) = cellstr('scanner');
fieldNames(n) = cellstr('edu_coded');
