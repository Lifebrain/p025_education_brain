...
label_lh_stats = paste(Sys.getenv("OUTPUT_DIR"),"02_mass-univariate_analyses/08_model04_overlap_lb_ukb/stats/combined_binarized.sig.lh.th13.abs.sig.masked.volume_C.mgh.label.stats",sep="/")
...
# Save plot
ggsave(filename = "04_LB_offset_cortical_volume_all.png", width = 12, height = 8, units = "cm")
...
# other variable like sex, scanner, etc, since the purpose here
# is visualization only.
mod2 <- gamm(value ~ education_group + s(age) + s(age,by=education_group) + scanner + sex, data = dat, random = list(id =~ 1))
summary(mod2$gam) 
# Plot the estimated function. Very nonlinear.
# This uses R's built-in plotting tool.
...
# Save plot
ggsave(filename = "04_LB_offset_cortical_volume_covariates_included.png", width = 12, height = 8, units = "cm")
ggsave(filename = "04_LB_offset_cortical_volume_covariates_included.svg", width = 12, height = 8, units = "cm")
